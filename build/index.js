...
let fieldsCache = new Map(); // key: `${databaseId}:${schema}:${tableName}`
// 初始化缓存
...
// 获取表的字段信息并缓存
async function getTableFields(databaseId, schema, tableName) {
...
// 根据自然语言查询找到最匹配的表
function findMatchingTable(query) {
    // 简单的匹配算法，可以根据需要改进
    for (const db of databasesCache) {
...
function generateSqlQuery(query, table, fields) {
    // 基本的 SQL 生成，可以根据需要改进
...
// 注册 Superset 查询工具
server.tool("query-superset", "执行 Superset 数据查询", {
...
// 注册获取数据库列表工具
server.tool("list-databases", "获取所有可用的数据库列表", {}, async () => {
    try {
        // 如果缓存为空，初始化缓存
        if (databasesCache.length === 0) {
            initializeCache();
...
// 注册获取表列表工具
server.tool("list-tables", "获取指定数据库的表列表", {
...
// 注册获取表字段工具
server.tool("list-fields", "获取指定表的字段列表", {
    databaseId: z.number().describe("数据库ID"),
